...
//                      });
  ...
  tick: function(){
    ...
    $('inGameMeterBar').innerHTML = this.templateManager.load('inGameMeterBar', {energy:this.scoreCalculator.score, hours:time[0]<10?"0"+time[0]:time[0], minutes:time[1]<10?"0"+time[1]:time[1], seconds:time[2]<10?"0"+time[2]:time[2]});
    var self = this;
    this.game.scene.reactor.push(this.game.scene.reactor.everySeconds(1), function(){self.tick()});
  }
